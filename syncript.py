...
def delete_remote(mgr: SSHManager, rel_paths: list[str],
    ...
    if dry_run:
        ...
        return
    # Batch delete with one `rm` command
    quoted = " ".join(f"'{REMOTE_ROOT}/{r}'" for r in rel_paths)
    cmd = f"rm -f {quoted}"
    try:
        mgr.exec(cmd, timeout=30)
        for rel in rel_paths:
            state.pop(rel, None)
            prog["deleted_r"].append(rel)
...
def run_sync(dry_run=False, verbose=False, force=False,
    ...
    try:
        ...
        n_del_r = len(plan["to_delete_r"])
        n_del_l = len(plan["to_delete_l"])
        ...
        if plan["to_delete_r"]:
            ...
            delete_remote(mgr, plan["to_delete_r"], dry_run, state, progress)

        if plan["to_delete_l"] and not dry_run:
            for rel in plan["to_delete_l"]:
                lpath = LOCAL_ROOT / rel
        ...
        elif plan["to_delete_l"] and dry_run:
            for rel in plan["to_delete_l"]:
